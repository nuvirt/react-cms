...
import { Layout, Wrapper, MarkdownContent } from 'components/layout'
  ...
  return (
    ...
    <BlogWrapper>
        {props.posts.map(post => (
          <div>
            <p>key={post.data.slug}</p>
            <p>href={`/patterns/${post.data.slug}`}</p>
...
export const getStaticPaths: GetStaticPaths = async function() {
  ...
  const contentDir = './content/patterns/'
...
export const getStaticProps: GetStaticProps = async function({
  ...
  try {
    const files = preview
      ...
      : await getLocalFiles('content/patterns')
    ...
    const posts = await Promise.all(
      ...
      files.map(async file => {
        ...
        return {
          data: { ...post.data.frontmatter, slug, fullPath },
          content: excerpt,
        }
      })
    )
